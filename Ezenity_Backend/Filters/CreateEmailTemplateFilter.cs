...
namespace Ezenity_Backend.Filters
{
    ...
    /// </summary>
    public class CreateEmailTemplateFilter : IOperationFilter
    {
        /// <summary>
        ...
        {
            if (operation.OperationId != "CreateEmailTemplate")
                return;
            ...
            if (operation.Responses.Any(response => response.Key == StatusCodes.Status200OK.ToString()))
                operation.Responses[StatusCodes.Status200OK.ToString()].Content.Add("application/ezenity.api.createemailtemplatetest+json", new OpenApiMediaType() { Schema = schema });
        }
    }
}
